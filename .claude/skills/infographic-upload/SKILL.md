...
- 用 **Read** 開啟上傳圖，**親眼確認**：
  ...
  - 無錯字、無誇大或絕對宣稱（推薦／第一／100%／完全不感染）。
  - **資料來源與日期：圖內沒標不算問題，不必回報、不必退回院長重做**（院長 2026-08-22 裁示）。
    改以 **figcaption 補足歸屬**，且**對應該承載頁標題列既有的引用來源，不要統一套用一句**
    （例：`child-fever` 台灣兒科醫學會／`influenza` 疾管署／`newborn-fever` 國民健康署＋臺灣兒科醫學會＋兒童健康手冊）。
  - 疫苗／醫療圖：合規紅線同 00 §4（不寫價格數字可入圖但需謹慎、非急診定位）。
- 有疑慮先回報院長，不逕自上稿。**沒讀過的圖不上傳。**
- **⚠️ 圖文不一致時，先判「哪一邊是更強的來源」再決定改哪邊（2026-09-09c 立）。** 本步驟的預設假設是「圖要對齊文」，但**院長交付的圖是醫師本人產出的醫療內容**：
  - **文是 session 在查不到一手資料時降級寫成的中性敘述** → **把文對齊圖**，並在交付說明標明「具名來自院長交付的圖，session 端未能開啟一手仿單核對」。
    先例：`nasal-flu-vaccine` 因 egress 封鎖 `cdc.gov.tw`／`info.fda.gov.tw`，把「水楊酸類用藥」寫成「特定用藥」；院長的圖明寫「阿斯匹靈／水楊酸類等用藥」→ 內文（可見層＋隱藏區＋llms 雙檔）補具名，**但維持圖上的框架**（「接種前要先告訴醫師」，不改寫成絕對禁忌）。
  - **文是既有已核可內容而圖疑似有誤** → 回報院長，不逕自改任何一邊。
  - ⚠️ 這種不一致**驗證器完全抓不到**（圖是二進位、內文合規、兩邊都不會亮），只能靠這一步逐項比對。
...
改檔前 `md5sum` 比對 byte-identical；**⚠️ `cp` 前先 `ls internal/archive/ | grep <YYYYMMDD>` 確認檔名沒被平行 session 佔用，
`cp` 後看一次 `git status`——顯示 `M` 就是覆蓋到別批的備份、`??` 才是新檔**。`md5sum` 只證明你複製對了，
證明不了檔名沒人在用；2026-09-09c 實際踩過，同日稍早的批已建過同名檔）→ 00 檔頭版本行（**先 `git fetch origin main`
