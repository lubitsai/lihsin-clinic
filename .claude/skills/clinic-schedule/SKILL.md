...
- 情境 B 另加：Google 商家「一般營業時間」、MainPi。（Chatbase 已於 2026-09-25 全面停用；線上小幫手重產見下方步驟，情境 A／B 都要做。）詳見 SOP 檔頭同名章節。
...
3. **同步預約系統班表**（官網為主）：
   ...
   > **限制：官網單日公告只寫得出「哪一診次、幾點到幾點」，寫不出「由誰看」。**
   > 「某醫師代診」與「加開雙診」都只存在於公告圖與說明文字，沒有結構化資料，
   > **同步一定漏掉**——不另外建，預約系統那天就只會開單診的名額。
   > 兩者都在預約系統後台「排班管理 → 臨時異動（只停一天）」建：
   > **雙診**按該診次的「＋ 加開醫師（雙診）」；**代診**用「日期例外」的醫師代診。
   > （常態雙診例如週日早診固定兩位，官網門診時間表本來就有，會自動同步，不要手建。）
   > 同步跑完會在 warnings 印出這條提醒，看到就照做。
