...
官網只是其中一個載體。**交付訊息結尾一律附本表，逐項標 ✅／⬜**，不可只回報官網做完就結束。
...
- **恢復日只有 ⑥**：門口那張要記得撕掉。①④ 自動過期；**⑤ BookNow 不必開回來**——只關休診當節的時段，不動到其他診次（院長 2026-08-23 說明）。⚠️ 自建系統不同（改的是週班表、對所有未來日期生效），切換後別套用 BookNow 的習慣。
