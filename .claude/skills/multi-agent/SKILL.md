...
1. 確認四份交接文件齊全,程式碼已 commit 並 push 到指定分支。
   **commit 訊息語言:本流程預設英文;但 `lihsin-clinic` repo 內依 `internal/00_專案總覽索引.md` §8-6
   (院長 2026-08-23 裁示)一律用繁體中文** —— 這是下方〈原則〉「各 repo 的 CLAUDE.md 規範優先」
   在本 repo 的具體落點,寫在這裡是因為動手 commit 的當下不會回頭讀原則。
2. 向使用者交付總結(繁體中文):
   - 架構摘要
   - 實作內容與檔案清單
   - 審查發現與處置
