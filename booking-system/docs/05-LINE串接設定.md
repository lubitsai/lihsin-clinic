...
> ⚠ **這份設定是上線的必要條件**（院長 2026-08-13 裁示取消簡訊後）：
> Login 沒設定，家長就無法線上預約或線上查詢；Messaging 沒設定，一則通知都發不出去。
...
5. **重要**：到 **LINE Login → Linked OA**，把診所的官方帳號連結上去。
   ...
   **沒有加好友就無法推播，而簡訊已取消——不加好友＝完全收不到預約通知**，所以這一步別跳過。
...
2. 到 **Messaging API** 分頁：
   ...
   - 開啟 **Use webhook**
   - 按 **Verify**，應回覆成功（用途：偵測民眾封鎖／解除封鎖官方帳號；被封鎖者推不出去，系統不再排入通知）
...
4. 封鎖官方帳號後到後台看該病人 → 應顯示非好友（代表 webhook 有生效）。
   此後這位家長收不到任何通知，需由櫃檯電話聯絡——這是取消簡訊後的必然結果，請櫃檯知悉。
...
| 情境 | 行為 |
|---|---|
| LINE 登入成功 | 建立／更新 `line_accounts`，開民眾 session（有效 2 小時） |
| 線上預約成立 | 病歷若是這次新建的（含同行家人），即自動綁定病人 ↔ LINE |
| 既有病歷、尚未綁定此帳號 | **擋下預約**並說明需櫃檯核對——不讓人冒用證件綁走病歷身分 |
| 一個 LINE 帳號多位家庭成員 | 各成員限制分開計算；加入既有家人需櫃檯以綁定代碼建立，解除則家長自己可做 |
| LINE 登入失敗或民眾取消授權 | 導回查詢頁；**沒有替代身分**，改走電話或現場掛號 |
| 民眾封鎖官方帳號 | webhook 標記為非好友，之後不再排入通知（櫃檯需電話聯絡） |
| 通知內容 | 一律不含完整證件號與病情資訊 |

> **為什麼既有病歷一定要櫃檯核對**（院長 2026-08-15 裁示）：綁定等於這個 LINE 帳號
> 從此看得到該病人的所有預約紀錄。原本以「證件＋生日＋手機三者相符」為關卡，
> 但這三項知道的人不只本人——家屬、學校、外流名單都可能有，等於讓人冒用證件
> 綁走病歷身分。**核對身分這件事只有櫃檯做得到，程式代替不了**，所以系統只負責
> 發一組一次性代碼，真正的關卡是櫃檯當面看健保卡。
