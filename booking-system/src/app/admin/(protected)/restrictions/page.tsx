...
export default async function RestrictionsPage() {
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-2xl font-bold text-sage-700">預約限制名單</h1>
      <p className="text-ink-700 text-sm">
        未到累計超過門檻自動加入；解除／暫時解除／重設由管理員於病人頁或此處操作，均需原因並留稽核。
      </p>
      <Card>
        {rows.length === 0 ? (
          <p className="text-ink-500">目前沒有限制紀錄。</p>
        ) : (
                  ...
                  className={`rounded px-2 py-0.5 text-sm font-bold ${
                    r.status === "ACTIVE"
                      ...
                      : r.status === "SUSPENDED"
                        ...
                        : "bg-sage-50 text-ink-500"
                  }`}
                >
                  {r.status === "ACTIVE" ? "生效中" : r.status === "SUSPENDED" ? "暫時解除" : "已解除"}
                </span>
                <Link href={`/admin/patients/${r.patientId}`} className="font-bold text-sage-700 underline underline-offset-2">
                  {r.patient.name}
                </Link>
                <span className="text-ink-700">{r.patient.idNumberMasked}</span>
                <span className="text-ink-500 text-sm">
                  未到 {r.patient.noShowCount} 次｜{r.type === "AUTO_NO_SHOW" ? "自動" : "人工"}｜
                  {r.createdAt.toISOString().slice(0, 10)}
                </span>
                <span className="text-ink-500 text-sm">{r.reason}</span>
