...
> 以下項目在 Claude Code 雲端環境無法代為執行,需由使用者手動完成;完成後可自行刪除本段。

- [ ] **設定 Firecrawl API key 環境變數 `FIRECRAWL_API_KEY`**
  - Claude Code 網頁環境:環境設定 → Environment variables 新增 `FIRECRAWL_API_KEY`。
  - 本機:於 `~/.zshrc` 或 `~/.bashrc` 加 `export FIRECRAWL_API_KEY=...` 後重開終端機。
  - 金鑰僅存環境變數,切勿寫入檔案或 commit;一旦外流立即於 Firecrawl Dashboard rotate。
  - 設定後需「重開 session」才會被 MCP server 載入生效。
- [ ] **刪除已合併的舊分支 `claude/claude-md-preferences-ah01ku`**
  - 於 GitHub PR 頁面點 **Delete branch**,或到 Branches 頁面刪除(GitHub 代理禁止從雲端 session 刪分支)。
- [ ] **開啟「PR 合併後自動刪除分支」**
  - repo Settings → General → Pull Requests → 勾選 **Automatically delete head branches**。
