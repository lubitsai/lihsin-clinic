...
#
# ── 2026-09-05i 擴充（院長裁示「FORBIDDEN_TERMS 補詞」）────────────────────
# 起因：09-05d 稽核在首頁可見層抓到 3 處醫療廣告風險（病人見證 ×2、療效宣稱、
#       自稱最高級），而本詞表**一條都沒攔到**——「最佳」在表內、「最值得信賴」
#       不在，且病人見證是**句型**、本表是**詞彙**清單，天生抓不到。
#       驗證器歷批全綠，只證明「已知禁語沒中」，不等於「沒有踩線」。
#
# ⚠️ 加詞前對全站 78 檔（可見層＋隱藏區）乾跑過，以下三個候選**刻意剔除**，
#    因為命中的全是 §4-1 允許的衛教成效語／醫學事實，加了會讓每批卡在誤報：
#      「最好的」→「解釋力最好的模型」（atopic-march，學說描述）
#      「最有效」→「全家一起做最有效」「勤洗手是最有效的方法」（國健署／CDC 建議）
#      「最強」  →「傳染力最強」×7（腸病毒等疾病特性）
#    通則：**擴充本表前一律先乾跑，不要憑直覺加「最X」**。
#
# ⚠️ 「見證」裸詞也刻意不加：「見證孩子成長」屬正當品牌用語，改收
#    「病人見證」「患者見證」兩個精確詞組，零誤報且仍擋得住真正的風險句型。
#
# 新增 12 詞的分類：
#   ① 自稱最高級（乾跑全站零命中）：最值得信賴／最專業／最完整／最先進／最厲害
#   ② 病人見證句型（醫療廣告最典型的高風險型態＝等同刊登感謝函、推薦文）：
#      病人說：「／患者說：「／家長說：「／病人見證／患者見證／感謝函
#   ③ 療效宣稱：治好了
#      ⚠️ 未來若要寫「氣喘不是治好了就不會再發作」這類**否定式衛教句**會被擋，
#         那是預期行為：停下來確認語意後提案加 VISIBLE_ALLOWLIST，不要刪本詞。
#
# ⛔ 首頁院長敘事段的病人見證 2 處經院長 2026-09-05 裁示**維持原樣**，
#    已逐句收進 VISIBLE_ALLOWLIST（見該表末段），依 02 R5 不重啟討論。
# ---------------------------------------------------------------------------
FORBIDDEN_TERMS = [
    "推薦", "最佳", "第一", "首選", "權威", "資深", "頂尖", "唯一", "少數",
    # ① 自稱最高級（2026-09-05i）
    "最值得信賴", "最專業", "最完整", "最先進", "最厲害",
    # ② 病人見證句型（2026-09-05i）
    "病人說：「", "患者說：「", "家長說：「", "病人見證", "患者見證", "感謝函",
    # ③ 療效宣稱（2026-09-05i）
    "治好了",
...
VISIBLE_ALLOWLIST = [
    ...
    "第一週",              # 「哺餵不足型（第一週）」（序數）
    "第一哩路",            # 「陪新手爸媽走過第一哩路」（比喻）
    "第一秒",              # 肺功能「第一秒用力呼氣量（FEV1）」醫學名詞與吹氣說明（時間序數；院長 2026-09-25af 核可）
    ...
    "小鹿醫師最佳助手",
    # --- 2026-09-05 院長裁示：首頁院長敘事段的病人見證 2 處「不改」 ---
    #     09-05d 稽核提報「病人見證＋療效宣稱」；院長裁示原文：
    #     「不改：『治好了』『藥很有效』＝病人見證＋療效宣稱」。
    #     ⚠️ 這兩條是**既有措辭的豁免、不是解禁**——09-05i 新增的 ② ③ 類禁語
    #     仍對「新寫的任何內容」生效（同 §4-3「5.0 星」的不外溢原則）。
    #     依 02 R5 不重啟討論；日後稽核掃到這兩句，直接引本註解結案。
    "您開的藥很有效，但又不會讓我昏昏欲睡",
    "您終於把我多年來的困擾治好了",
]
