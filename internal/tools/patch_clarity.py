...
# -*- coding: utf-8 -*-
"""
立欣診所 Microsoft Clarity 共用腳本全站掛載（外科式字串插入，比照 patch_cta.py）
================================================================================
對指定目錄下所有 .html 插入一行：
  <script src="/clarity.js" defer></script>（插在 </head> 前）

安全機制：
  - 冪等：頁面已含 clarity.js 即整頁跳過（可重複執行）
  - 排除清單：offline.html、404.html（沿 patch_pwa／patch_cta 慣例）
  - 只用 str.find 精準插入，不重新序列化 HTML

用法：
  python3 internal/tools/patch_clarity.py <目標目錄> [--dry]
"""
import os
import sys

CLARITY_TAG = '<script src="/clarity.js" defer></script>'
...
def patch_file(path, dry=False):
    ...
    new = content[:head_end] + ' ' + CLARITY_TAG + '\n' + content[head_end:]
    delta = len(new) - len(content)
    if not dry:
        with open(path, 'w', encoding='utf-8') as f:
            f.write(new)
    return '✓ clarity 掛載', delta
