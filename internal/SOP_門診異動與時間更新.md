...
| **⑥** | **診所門口實體公告** | 櫃檯 | 列印 A4 貼門口（`internal/tools/make_door_notice.py`）；**恢復日撕掉** | 已經出門、走到門口的家長只看得到這張——**最後一道防線**，也是唯一擋得住「沒在用網路的家長」的載體 |
...
**恢復日只有一件事**：**⑥ 門口那張要記得撕掉**——六項裡唯一需要人回頭處理的。
- ①公告卡 `data-expires` 自動下架、④ 特殊營業時間自動過期，都不必管。
- **⑤ BookNow 不需要開回來**（院長 2026-08-23 說明）：只關掉休診當節的那些時段，
  **不會動到其他診次、也不影響之後同一星期幾的班表**，所以沒有「恢復」這個動作。
